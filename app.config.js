// app.config.js
export default ({ config }) => {
    return {
      ...config,
      name: "Juno2",
      slug: "kiko",
      version: "1.3.0",
      orientation: "portrait",
      icon: "./assets/images/icon.png",
      scheme: "juno2",
      userInterfaceStyle: "automatic",
      newArchEnabled: false,
      splash: {
        image: "./assets/images/splash-icon.png",
        resizeMode: "contain",
        backgroundColor: "#020817"
      },
      assetBundlePatterns: [
        "**/*"
      ],
      ios: {
        supportsTablet: true,
        bundleIdentifier: "com.pierretulle.juno2",
        buildNumber: "3"
      },
      android: {
        adaptiveIcon: {
          foregroundImage: "./assets/images/logo3.png",
          backgroundColor: "#020817"
        },
        package: "com.pierretulle.juno2",
        permissions: [ // Assure-toi que cette section existe et ajoute la permission AD_ID
          "INTERNET",
          "VIBRATE",
          "MODIFY_AUDIO_SETTINGS",
          "READ_EXTERNAL_STORAGE",
          "RECORD_AUDIO",
          "SYSTEM_ALERT_WINDOW",
          "WRITE_EXTERNAL_STORAGE",
          "com.google.android.gms.permission.AD_ID" // <--- AJOUTE CETTE LIGNE
        ],
        googleServicesFile: "./google-services.json"
      },
      web: {
        bundler: "metro",
        output: "static",
        favicon: "./assets/images/favicon.png"
      },
      plugins: [
        [
          "expo-build-properties",
          {
            "android": {
              "kotlinVersion": "1.9.25"
            }
          }
        ],
        "expo-router",
        [
          "expo-splash-screen",
          {
            "image": "./assets/images/splash-icon.png",
            "imageWidth": 200,
            "resizeMode": "contain",
            "backgroundColor": "#020817"
          }
        ],
        [
          "react-native-google-mobile-ads",
          {
            "androidAppId": "ca-app-pub-7809209690404525~1711130974",
            "iosAppId": "ca-app-pub-7809209690404525~1711130974"
          }
        ],
        "@react-native-firebase/app"
      ],
      experiments: {
        typedRoutes: true
      },
      owner: "pierretulle",
      description: "Découvrez l'histoire en devinant l'ordre chronologique des événements. Un jeu ludique et éducatif pour tous âges.",
      primaryColor: "#1D5F9E",
      privacy: "public",
      platforms: [
        "android",
        "ios"
      ],
      githubUrl: "https://github.com/pierretulle/quandi",
      updates: {
        fallbackToCacheTimeout: 0,
        url: "https://u.expo.dev/3cbda57c-1ec1-4949-af06-9e933dbc0050"
      },
      runtimeVersion: {
        policy: "appVersion"
      },
      extra: {
        ...(config.extra || {}),
        router: {
          origin: false
        },
        eas: {
          projectId: "3cbda57c-1ec1-4949-af06-9e933dbc0050"
        },
        APP_VARIANT: process.env.EXPO_PUBLIC_APP_VARIANT || 'development',
      },
    };
  };