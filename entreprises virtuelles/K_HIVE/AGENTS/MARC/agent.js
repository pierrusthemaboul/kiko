...
async function generateHookWithAI(clip, maxRetries = 3) {
    ...
    for (let i = 0; i < maxRetries; i++) {
        try {
            ...
            const prompt = `Tu es un créateur TikTok. Génère UN SEUL hook accrocheur (max 55 chars) pour cette vidéo sur l'événement historique:

Événement: ${clip.evenement.titre}
Date: ${clip.evenement.date}
Description: ${clip.evenement.description}

Le jeu s'appelle Timalaus et demande "AVANT ou APRÈS?".
