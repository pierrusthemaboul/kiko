...
const InterstitialAd = {
  createForAdRequest: (unitId?: string, options?: any) => ({
    addAdEventListener: (event: any, handler: (...args: any[]) => void) => () => {},
    load: () => {},
    show: () => {},
    loaded: false,
  }),
...
export function useAds({
  ...
  const canShowAd = useCallback((adType: 'levelUp' | 'gameOver' | 'generic' = 'generic') => {
    ...
    if (adType === 'levelUp' && (currentLevel === 2 || currentLevel === 6 || currentLevel % 5 === 0)) {
      // ✅ MODIF : Désactiver la pub après le niveau 1 en mode DEV pour les enregistrements
      if (__DEV__ && currentLevel === 2) {
        adLog('log', `canShowAd (${adType}): Bloquée en mode DEV pour enregistrement (level: ${currentLevel})`);
        return false;
      }
  ...
  useEffect(() => {
    ...
    const unsubGenericError = genericInterstitial.addAdEventListener(AdEventType.ERROR, (error: any) => {
    ...
    const unsubLevelUpError = levelUpInterstitial.addAdEventListener(AdEventType.ERROR, (error: any) => {
      const errorCode = (error as any)?.code ?? 'unknown_code';
      const errorMessage = error?.message ?? 'unknown_message';
    ...
    const unsubGameOverError = gameOverInterstitial.addAdEventListener(AdEventType.ERROR, (error: any) => {
      const errorCode = (error as any)?.code ?? 'unknown_code';
      const errorMessage = error?.message ?? 'unknown_message';
    ...
    const unsubRewardedError = rewardedAd.addAdEventListener(AdEventType.ERROR, (error: any) => {
      const errorCode = (error as any)?.code ?? 'unknown_code';
      const errorMessage = error?.message ?? 'unknown_message';
    ...
    const unsubRewardedEarned = rewardedAd.addAdEventListener(RewardedAdEventType.EARNED_REWARD, (reward: any) => {
      if (processingRewardRef.current) {
