    ...
    for (const item of queueItems) {
        ...
        while (attempt <= MAX_RETRIES && !success) {
            ...
            try {
                ...
                fs.writeFileSync(path.resolve('./tools/machine_a_evenements/AGENTS/TRINITY/STORAGE/INPUT/task.json'), JSON.stringify(task, null, 2));

                console.log(`⏳ [TRINITY] Génération d'image (Tentative ${attempt})...`);
                execSync(`node agent.js`, { cwd: path.resolve('./tools/machine_a_evenements/AGENTS/TRINITY'), stdio: 'inherit' });

                // --- VERITAS ---
                console.log("⚖️ [VERITAS] Validation...");
                execSync(`node agent.js`, { cwd: path.resolve('./tools/machine_a_evenements/AGENTS/VERITAS'), stdio: 'inherit' });

                const veritasResult = JSON.parse(fs.readFileSync(path.resolve('./tools/machine_a_evenements/AGENTS/VERITAS/STORAGE/OUTPUT/veritas_result.json'), 'utf8'));
