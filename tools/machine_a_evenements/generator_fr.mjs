...
async function generateNewEvents() {
  ...
  const rules = fs.readFileSync('tools/machine_a_evenements/choix_des_evenements.md', 'utf8');
  const existingTitles = JSON.parse(fs.readFileSync('tools/machine_a_evenements/existing_titles.json', 'utf8'));
...
`;
  ...
  try {
    ...
    fs.writeFileSync('tools/machine_a_evenements/proposition_batch.json', JSON.stringify(data, null, 2));
    console.log("\n📁 Proposition sauvegardée dans 'tools/machine_a_evenements/proposition_batch.json'");
