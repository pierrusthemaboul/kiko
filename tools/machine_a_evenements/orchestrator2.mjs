...
async function startSession() {
    ...
    try {
        execSync(`node agent.js`, {
            cwd: path.resolve('./tools/machine_a_evenements/AGENTS/ANALYST'),
            stdio: 'inherit'
        });

        const briefPath = path.resolve('./tools/machine_a_evenements/AGENTS/ANALYST/STORAGE/OUTPUT/analyst_brief.json');
    ...
    const sessionPath = path.resolve('./tools/machine_a_evenements/AGENTS/session_history.json');
...
async function executeOrchestration(TARGET, THEME, BATCH_SIZE, MAX_ATTEMPTS) {
    ...
    while (validatedPillars.length < TARGET && attempts < MAX_ATTEMPTS) {
        ...
        try {
            ...
            execSync(`node agent.js ${BATCH_SIZE} "${THEME}"`, {
                cwd: path.resolve('./tools/machine_a_evenements/AGENTS/GENESIS2'),
                stdio: 'inherit'
            });
            totalGenerated += BATCH_SIZE;

            // Copy GENESIS2 output to GENESIS output path (so SENTINEL can read it)
            const genesis2Output = path.resolve('./tools/machine_a_evenements/AGENTS/GENESIS2/STORAGE/OUTPUT/genesis2_raw_batch.json');
            const genesisInput = path.resolve('./tools/machine_a_evenements/AGENTS/GENESIS/STORAGE/OUTPUT/genesis_raw_batch.json');
            ...
            execSync(`node agent.js`, {
                cwd: path.resolve('./tools/machine_a_evenements/AGENTS/SENTINEL'),
                stdio: 'inherit'
            });
            ...
            execSync(`node agent.js`, {
                cwd: path.resolve('./tools/machine_a_evenements/AGENTS/QUALITY_GATE'),
                stdio: 'inherit'
            });

            // Read results from QUALITY_GATE
            const qualityPath = 'tools/machine_a_evenements/AGENTS/QUALITY_GATE/STORAGE/OUTPUT/quality_gate_passed.json';
            const sentinelAuditPath = 'tools/machine_a_evenements/AGENTS/SENTINEL/STORAGE/OUTPUT/sentinel_audit_report.json';
            const qualityRejectPath = 'tools/machine_a_evenements/AGENTS/QUALITY_GATE/STORAGE/OUTPUT/quality_gate_rejected.json';
    ...
    const outputPath = 'tools/machine_a_evenements/orchestrator2_result.json';
    fs.writeFileSync(outputPath, JSON.stringify(resultData, null, 2));
    fs.writeFileSync('tools/machine_a_evenements/orchestrator_result.json', JSON.stringify(resultData, null, 2));
    ...
    if (allFinalEvents.length > 0) {
        ...
        try {
            ...
            execSync(`node agent.js`, {
                cwd: path.resolve('./tools/machine_a_evenements/AGENTS/ARTISAN2'),
                stdio: 'inherit'
            });

            // STEP 6: REXP (reuse existing - export to Supabase)
            // We need to copy ARTISAN2 output to ARTISAN path so REXP can read it
            const artisan2Output = path.resolve('./tools/machine_a_evenements/AGENTS/ARTISAN2/STORAGE/OUTPUT/artisan2_finished_products.json');
            const artisanOutput = path.resolve('./tools/machine_a_evenements/AGENTS/ARTISAN/STORAGE/OUTPUT/artisan_finished_products.json');
            ...
            execSync(`node agent.js`, {
                cwd: path.resolve('./tools/machine_a_evenements/AGENTS/REXP'),
                stdio: 'inherit'
            });
