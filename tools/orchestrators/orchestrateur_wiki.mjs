...
const MANIFEST_PATH = 'c:/Users/Pierre/kiko/plan_de_moisson.json';

async function orchestrerUsine() {
    console.log("\n🏭 Démarrage de l'Orchestrateur Global de l'Usine Timalaus...");

    let plan;
    try {
        plan = JSON.parse(fs.readFileSync(MANIFEST_PATH, 'utf8'));
    } catch (e) {
        console.error("❌ Fichier plan_de_moisson.json introuvable ou mal formé.", e.message);
        return;
    }

    console.log(`📋 Plan de moisson chargé: ${plan.length} cibles Wikipedia.\n`);
