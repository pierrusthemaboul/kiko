  ...
  const pickEventForLevel = useCallback(
    (targetLevel: PrecisionLevel): Event | null => {
      ...
      for (let i = 0; i < pool.length; i += 1) {
        ...
        if (!used.has(item.id)) {
          // Check that the date is different from currentEvent
          const currentYear = currentEvent ? extractYear(currentEvent.date) : null;
          const candidateYear = extractYear(item.date);

          // Use a strict date check if possible, fallback to year if necessary
          const currentFullDate = currentEvent?.date;
          const candidateFullDate = item.date;

          if (currentFullDate && candidateFullDate && currentFullDate === candidateFullDate) {
            continue;
          }
