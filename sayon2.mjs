// ==============================================================================
// sayon_hybrid_strategy.mjs - VERSION OPTIMALE - ÉQUILIBRE QUANTITÉ/QUALITÉ
// CONFIGURATION OPTIMALE :
// - Claude 3.5 Sonnet : Génération événements + Vérification + Enrichissement
// - GPT-4o : Génération prompts visuels OPTIMISÉS (créativité + année/période)
// - GPT-4o-mini : Validation images INTELLIGENTE (économique + anti-date/titre)
// - Flux-schnell : Génération images avec prompts PARFAITS
// OBJECTIF : 35-45/50 événements (70-90%), qualité 8-9/10, robustesse maximale
// ==============================================================================
...
const BATCH_SIZE = 4; // 🎯 OPTIMAL: Équilibre qualité/quantité
const MIN_VALIDATION_SCORE = 4; // 🎯 OPTIMAL: Maintient qualité élevée (vos images = 9/10)
...
async function callClaude(prompt, options = {}) {
    ...
    try {
        ...
        console.log(`      ✅ [CLAUDE] Réponse reçue (${response.content[0].text.length} chars)`);
        return response.content[0].text;
        
    } catch (error) {
        console.error(`      ❌ [CLAUDE] Erreur:`, error.message);
        
        // 🎯 ROBUSTESSE: Retry automatique pour erreurs temporaires
        if ((error.message.includes('Connection error') || 
             error.message.includes('rate_limit') || 
             error.message.includes('overloaded') ||
             error.message.includes('timeout')) && retryAttempt < 3) {
            const waitTime = retryAttempt * 5000; // 5s, 10s, 15s
...
async function callOpenAI(prompt, options = {}) {
    ...
    console.log(`      🤖 [OPENAI] Appel ${model} (${prompt.length} chars)${retryAttempt > 1 ? ` - Retry ${retryAttempt}/3` : ''}`);
    ...
    try {
        ...
        console.error(`      ❌ [OPENAI] Erreur:`, error.message);
        
        // 🎯 ROBUSTESSE: Retry automatique
        if ((error.message.includes('rate_limit') || 
             error.message.includes('timeout') ||
             error.message.includes('Connection')) && retryAttempt < 3) {
            const waitTime = retryAttempt * 4000;
            console.log(`      🔄 [OPENAI] Retry automatique dans ${waitTime/1000}s...`);
            await new Promise(resolve => setTimeout(resolve, waitTime));
            return await callOpenAI(prompt, { ...options, retryAttempt: retryAttempt + 1 });
        }
...
async function callOpenAIWithImage(prompt, imageUrl, options = {}) {
    const {
...
function isDuplicate(titre) {
    ...
    // 🎯 OPTIMAL: Vérification stricte mais intelligente pour éviter rejets excessifs
...
async function generateEventBatchWithClaude(startYear, endYear, count, attemptNumber = 1) {
    console.log(`   📦 [CLAUDE] Génération de ${count} événements (tentative ${attemptNumber})...`);
    ...
    const sampleExisting = periodExistingTitles.slice(0, 15).join('", "'); // 🎯 OPTIMAL: Réduit pour prompt plus efficace
...
🚫 ÉVÉNEMENTS INTERDITS (déjà en base) :
"${sampleExisting}"
...
3. ZÉRO DOUBLON avec la liste d'interdiction
...
PRIORITÉ ABSOLUE : Précision historique + DIVERSITÉ GÉOGRAPHIQUE pour éviter doublons.`;

    try {
        const responseText = await callClaude(prompt, {
            model: HYBRID_CONFIG.eventGeneration,
            max_tokens: 2200, // 🎯 OPTIMAL: Suffisant pour 4 événements de qualité
            temperature: 0.25 // 🎯 OPTIMAL: Équilibre précision/créativité
        });
        ...
        console.log(`      📊 [CLAUDE] JSON extrait: ${jsonText.substring(0, 200)}...`);
        ...
        if (!batchData.events || !Array.isArray(batchData.events)) {
            ...
            if (attemptNumber < 3) {
                return await generateEventBatchWithClaude(startYear, endYear, count, attemptNumber + 1);
            }
            return [];
        }
        ...
        batchData.events.forEach(event => {
            ...
            // 🎯 OPTIMAL: Regex complète pour caractères français
        ...
        console.log(`      ✅ [CLAUDE] Lot généré: ${validEvents.length} événements uniques après pré-vérification`);
        console.log(`      🔍 [CLAUDE] Pré-vérification: ${batchData.events.length - validEvents.length} doublons évités`);
        ...
        console.error(`      ❌ [CLAUDE] Erreur génération:`, error.message);
...
async function verifyEventBatchWithClaude(events) {
    console.log(`   🕵️ [CLAUDE] Vérification historique approfondie...`);
...
PRIORITÉ : Précision historique absolue avec dates vérifiées.`;
    ...
    try {
        const responseText = await callClaude(prompt, {
            model: HYBRID_CONFIG.historicalVerification,
            max_tokens: 1000,
            temperature: 0.1
        });
        ...
        events.forEach((event, index) => {
            ...
            if (validation && validation.isValid && validation.dateCorrect) {
                validEvents.push(event);
                console.log(`      ✅ [CLAUDE] "${event.titre}" (${event.year}) - Validé (${validation.confidence})`);
            } else {
                invalidEvents.push({ event, reason: validation?.reason || 'Non vérifié par Claude' });
                console.log(`      ❌ [CLAUDE] "${event.titre}" (${event.year}) - ${validation?.reason || 'Erreur validation'}`);
            }
        });
        
        console.log(`      📊 [CLAUDE] Vérification: ${validEvents.length}/${events.length} validés`);
        
        return { validEvents, invalidEvents };
        
    } catch (error) {
        console.error(`      ❌ [CLAUDE] Erreur vérification:`, error.message);
...
async function enrichEventWithClaude(event, attemptNumber = 1) {
    console.log(`      🔍 [CLAUDE] Enrichissement contextuel: "${event.titre}" (${event.year})...`);
    
    if (attemptNumber > 1) {
        console.log(`      🔄 [CLAUDE] Tentative ${attemptNumber}/2 après erreur connexion`);
    }
...
EXIGENCE : Exactitude historique absolue pour ${event.year}.`;
    ...
    try {
        const responseText = await callClaude(prompt, {
            model: HYBRID_CONFIG.contextEnrichment,
            max_tokens: 600,
            temperature: 0.3
        });
        ...
        console.log(`      📊 [CLAUDE] Enrichissement reçu:`);
        ...
        console.error(`      ❌ [CLAUDE] Erreur enrichissement:`, error.message);
        
        // 🎯 OPTIMAL: Retry automatique pour erreurs de connexion
        if (error.message.includes('Connection error') && attemptNumber < 2) {
            console.log(`      🔄 [CLAUDE] Retry enrichissement (erreur connexion)...`);
            await new Promise(resolve => setTimeout(resolve, 3000));
            return await enrichEventWithClaude(event, attemptNumber + 1);
        }
...
async function generateOptimizedFluxPromptWithGPT(enrichedEvent) {
    console.log(`      🎨 [GPT-4O] Génération prompt visuel optimisé pour "${enrichedEvent.titre}"...`);
    ...
    const promptForGPT = `Tu es un expert en prompts pour Flux-schnell. Génère le MEILLEUR prompt possible pour illustrer cet événement historique.
...
⚡ RÉPONDS UNIQUEMENT avec le prompt Flux-schnell OPTIMAL incluant "${enrichedEvent.year}" et "${epoch} period", MAXIMUM ${FLUX_SCHNELL_LIMITS.TARGET_WORDS} MOTS.`;
    ...
    try {
        ...
        console.log(`      📊 [GPT-4O] Prompt initial: "${cleanPrompt}" (${initialWords} mots)`);
        ...
        console.log(`      📊 [GPT-4O] Prompt final OPTIMISÉ: "${finalPrompt}"`);
        ...
        console.error(`      ❌ [GPT-4O] Erreur génération prompt:`, error.message);
...
async function generateImageEnhanced(prompt, event) {
    ...
    // Configuration Flux-schnell OPTIMISÉE pour événements historiques
    const fluxConfig = {
        prompt,
...
// ==============================================================================
// VALIDATION INTELLIGENTE GPT-4O-MINI
// ==============================================================================

async function validateImageWithGPTMini(event, imageUrl) {
    console.log(`   🔍 [GPT-4O-MINI] Validation intelligente pour ${event.year}...`);
...
JSON OBLIGATOIRE:
{
  ...
  "reason": "explication détaillée de l'évaluation"
    ...
    try {
        ...
        if (result.hasForbiddenText) {
            console.log(`      ❌ [GPT-4O-MINI] REJET: Texte interdit détecté (date ${event.year} ou titre "${event.titre}")`);
            return false;
        }
        ...
        // Validation finale basée sur score et critères - OPTIMAL
...
async function processEventWithHybridStrategy(event) {
    ...
    // Phase 1: Enrichissement avec Claude 3.5 Sonnet
    console.log(`      📚 Phase 1: [CLAUDE] Enrichissement contextuel...`);
    ...
    for (let attempt = 1; attempt <= MAX_IMAGE_ATTEMPTS && !successfullyCreated; attempt++) {
        ...
        try {
            // Phase 2a: Génération prompt avec GPT-4o
            const optimizedPrompt = await generateOptimizedFluxPromptWithGPT(enrichedEvent);
            ...
            // Phase 3: Validation avec GPT-4o-mini
            const isValid = await validateImageWithGPTMini(enrichedEvent, imageUrl);
            
            if (isValid) {
                try {
                    console.log(`      📤 [HYBRID] Upload vers Supabase...`);
                    const uploadedUrl = await uploadImageToSupabase(imageUrl, event.titre);
                    const finalEvent = enrichAndFinalizeEvent(enrichedEvent, uploadedUrl, optimizedPrompt);
                    await insertValidatedEvent(finalEvent);
                    
                    addToCache(event.titre);
                    console.log(`      ✅ [HYBRID] Événement créé avec succès !`);
                    console.log(`      📊 Stratégie: Claude→GPT-4o→Flux→GPT-mini`);
                    ...
                    if (attempt === MAX_IMAGE_ATTEMPTS) {
                        try {
                            const finalEvent = enrichAndFinalizeEvent(enrichedEvent, imageUrl, optimizedPrompt);
                            await insertValidatedEvent(finalEvent);
                            addToCache(event.titre);
    ...
    try {
        ...
        const finalEvent = enrichAndFinalizeEvent(enrichedEvent, defaultImageUrl, "Image par défaut");
        await insertValidatedEvent(finalEvent);
...
async function uploadImageToSupabase(imageUrl, eventTitle) {
    ...
    const fileName = `optimal_${eventTitle.toLowerCase().replace(/[^a-z0-9]/g, '_').substring(0, 30)}_${Date.now()}.webp`;
...
function enrichAndFinalizeEvent(enrichedEvent, imageUrl, illustrationPrompt) {
    ...
    return {
        ...
        code: `opt${Date.now().toString().slice(-6)}`,
        date_precision: 'year',
        ecart_temps_min: 50,
        frequency_score: 0,
...
async function insertValidatedEvent(finalEvent) {
    ...
    if (error) {
        if (error.code === '23505') {
            finalEvent.code = `opt${Date.now().toString().slice(-6)}${Math.floor(Math.random()*100)}`;
...
async function processBatchHybrid(startYear, endYear, batchSize, batchNumber) {
    console.log(`\n📦 === LOT ${batchNumber} HYBRIDE OPTIMAL (${batchSize} événements) ===`);
    
    // Phase 1: Génération avec Claude 3.5 Sonnet
    const events = await generateEventBatchWithClaude(startYear, endYear, batchSize, batchNumber);
    if (events.length === 0) {
        console.log("❌ [CLAUDE] Échec génération");
        return [];
    }
    
    // Phase 2: Vérification avec Claude 3.5 Sonnet
...
async function main() {
    console.log("\n🚀 === SAYON VERSION OPTIMALE - ÉQUILIBRE QUANTITÉ/QUALITÉ ===");
    console.log("🎯 Configuration IA hybride:");
    console.log("   🧠 Claude 3.5 Sonnet: Génération + Vérification + Enrichissement");
    console.log("   🎨 GPT-4o: Génération prompts visuels (créativité)");
    console.log("   💰 GPT-4o-mini: Validation images (économique)");
    console.log("   🖼️ Flux-schnell: Génération images");
    console.log("📊 Objectifs:");
    console.log("   📈 Taux de réussite: 18% → 70-90% (+300-400%)");
    console.log("   💰 Réduction coûts: -80% grâce à moins d'échecs");
    console.log("   ⏱️ Temps optimisé: Moins de retry, plus d'efficacité");
    console.log("   🎯 Qualité maintenue: 8-9/10");
    
    console.log("\n🎯 OPTIMISATIONS VERSION OPTIMALE:");
    ...
    console.log("   ✅ 3. Instructions dates Claude renforcées");
    ...
    console.log("   ✅ 6. Retry automatique (Claude, OpenAI, robustesse maximale)");
    console.log("   ✅ 7. Diversité géographique maximale (15 types d'événements)");
    console.log("   ✅ 8. Limite lots étendue (25 au lieu de 8)");
    ...
    while (createdCount < targetCount && batchNumber < 25) { // 🎯 OPTIMAL: Limite augmentée
        batchNumber++;
        const remainingEvents = targetCount - createdCount;
        const currentBatchSize = Math.min(BATCH_SIZE, remainingEvents);
        
        try {
            console.log(`\n🚀 [HYBRID] Début lot ${batchNumber} avec stratégie optimale...`);
            ...
            console.log(`\n📊 BILAN LOT ${batchNumber} HYBRIDE OPTIMAL:`);
            ...
            // 🎯 OPTIMAL: Continue même en cas d'erreur de lot
    ...
    console.log(`\n🎉 === TRAITEMENT HYBRIDE OPTIMAL TERMINÉ ===`);
    ...
    console.log(`🤖 Stratégie hybride: Claude 3.5 + GPT-4o + Flux-schnell + GPT-4o-mini`);
    console.log(`💰 Économies estimées: ${realFinalSuccessRate > 60 ? 'Substantielles' : realFinalSuccessRate > 40 ? 'Modérées' : 'Faibles'} vs approche monolithique`);
    console.log(`🎯 Qualité: Précision Claude + Créativité GPT + Économie optimisée + Validation intelligente`);
    
    // 🎯 OPTIMAL: Diagnostic si taux faible
    if (realFinalSuccessRate < 60) {
        ...
        console.log(`\n🎊 EXCELLENT RÉSULTAT ! Taux > 60% atteint avec qualité préservée`);
    }
