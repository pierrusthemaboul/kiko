...
async function searchFacoveInLogs() {
  console.log('Searching for "facove" in log messages or data...')
  
  const { data: logs, error } = await supabase
    .from('remote_debug_logs')
    .select('*')
    .or('message.ilike.%facove%,data.ilike.%facove%')
    .order('created_at', { ascending: false })
  
  if (error) console.error(error)
  else console.log('Logs containing "facove":', JSON.stringify(logs, null, 2))
}
